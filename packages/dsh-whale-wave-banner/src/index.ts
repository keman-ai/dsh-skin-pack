/**
 * The Whale Wave Banner skin · host half.
 *
 ...
 *
 * @module dsh-whale-wave-banner
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { WAVE_BANNER } from './cover.generated.ts'

/** Plugin name (the `name` of the loader entry). */
export const name = 'whale-wave'
 ...
 */
export const COVER_ROUTE = '/skin-cover/whale-wave.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(WAVE_BANNER.slice(WAVE_BANNER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Whale Wave on install; on by default. Turned off, it only registers without applying, leaving the user to pick it under Settings → Appearance.
   *
