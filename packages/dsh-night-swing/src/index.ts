/**
 * The Night Swing skin · host half.
 *
 ...
 *
 * @module dsh-night-swing
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { SWING_COVER } from './cover.generated.ts'
 ...
 */
export const COVER_ROUTE = '/skin-cover/swing.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(SWING_COVER.slice(SWING_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Night Swing on install; on by default. Turned off, it only registers without applying, leaving the user to pick it under Settings → Appearance.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.logger.info('[swing] Night Swing mounted (%s)', mode)
}
