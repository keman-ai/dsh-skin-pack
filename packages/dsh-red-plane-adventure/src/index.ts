/**
 * The Red Plane Adventure skin · host half.
 *
 ...
 *
 * @module dsh-red-plane-adventure
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { REDPLANE_COVER } from './cover.generated.ts'

/** Plugin name (the `name` of the loader entry). */
export const name = 'redplane'
 ...
 */
export const COVER_ROUTE = '/skin-cover/redplane.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(REDPLANE_COVER.slice(REDPLANE_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Red Plane Adventure on install; on by default. Turned off, it only registers without applying, leaving the user to pick it under Settings → Appearance.
   *
