/**
 * The right-hand dock: a reduced red-plane cover plus this session's real state.
 *
 * The prototype's right column has four cards: Current Session / Harness Systems / Flight Modes / Flight Energy.
 ...
 * `Flight Modes` cards and the 90% of `Flight Energy` are pure decoration with hardcoded numbers; the harness has no such projection, and nothing is fabricated.
 *
...
export function RedPlaneStatusDock() {
      ...
      {open && (
        <div className={css.body}>
          ...
          <div className={css.scroll}>
            ...
            */}
            <div className={css.cover} style={{ backgroundImage: 'var(--redplane-cover)' }}>
              <span className={css.coverName}>Red Plane Adventure</span>
            </div>
                    ...
                    <li key={`${call.name}-${index}`} className={css.logRow} data-state={callState(call)}>
                      ...
                      <span className={css.logMeta}>
                        {call.running === true
                          ...
                          : call.failed === true
                            ? call.ms === undefined ? 'went down' : `went down · ${formatDuration(call.ms)}`
                            : call.ms === undefined ? 'landed' : `landed · ${formatDuration(call.ms)}`}
                      </span>
                    </li>
                  ))}
                </ul>
                {moreTools > 0 && <p className={css.hint}>{`${moreTools} earlier flight(s) not shown`}</p>}
                {/*
