...
/**
 * 封面地址。由 host 半在 `/skin-cover/emerald.webp` 上提供（见 src/index.ts 的 COVER_ROUTE）。
 *
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * 挂载不区分皮肤是否激活，可见性交给 CSS（`body[data-dsh-emerald]` 才 display）——
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[emerald] 已有皮肤占了自动应用名额（%s），本套改为待选', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   *（`isThemePreference('emerald')` 是 false，第三方 id 根本不进持久化），而 Host 快照
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[emerald] 自动应用失败，可到「设置 → 皮肤市场」手动切换', error)
      }
      return
    }
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[emerald] 品牌位 %s 接管失败，保留官方标', slot.name, error)
        return () => {}
      }
    }))
  }
