/**
 * The Emerald Megacity skin · host half.
 *
 ...
 *
 * @module dsh-emerald-megacity
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { EMERALD_COVER } from './cover.generated.ts'

/** Plugin name (the `name` of the loader entry). */
export const name = 'emerald'
 ...
 */
export const COVER_ROUTE = '/skin-cover/emerald.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(EMERALD_COVER.slice(EMERALD_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Emerald Megacity on install; on by default. Turned off, it only registers without applying, leaving the user to pick it under Settings → Appearance.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.logger.info('[emerald] Emerald Megacity mounted (%s)', mode)
}
