...
import { SeasideMark, SeasideName } from './Brand.tsx'
import { SeasideStatusDock } from './StatusDock.tsx'
import { MistEnergyRail } from './EnergyRail.tsx'
import { SeasideStatusProbe } from './StatusProbe.tsx'
import { SEASIDE_TOKENS } from './tokens.ts'
import './seaside.module.css'

export { SEASIDE_PALETTE, SEASIDE_TOKENS } from './tokens.ts'
...
export const BODY_ATTRIBUTE = 'data-dsh-seaside'
...
const COVER_VARIABLE = '--seaside-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-seaside-dock-open'
const DOCK_STORAGE_KEY = 'seaside'

/**
 * The cover URL, served by the host half at `/skin-cover/seaside.webp` (see COVER_ROUTE in src/index.ts).
 *
 ...
 */
const COVER_URL = '/skin-cover/seaside.webp?v=0.1.0'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: SeasideMark },
  { name: 'sidebar.brand.name', component: SeasideName },
  { name: 'conversation.hero.brand.mark', component: SeasideMark },
] as const
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * Mounting does not depend on whether the skin is active; visibility is left to CSS (display only under `body[data-dsh-seaside]`) —
   * The rule is "not active means not present", so no half-built UI shows during the window before the skin takes effect.
   */
  ctx.effect(() => mountDock(), 'seaside: status dock')
  ...
  ctx.effect(() => ctx.slots.inject('conversation.composer.dock', () => ctx.slots.register({
    ...
    order: 100,
  }, SeasideStatusProbe)), 'seaside: status probe')
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    id: 'seaside-energy',
    order: 100,
  }, MistEnergyRail)), 'seaside: energy rail')
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'light', tokens: SEASIDE_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'seaside: theme + cover + brand')
}
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[seaside] another skin already claimed the auto-apply slot (%s); this one waits to be picked', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   * (`isThemePreference('seaside')` is false and third-party ids never reach persistence), and when the Host snapshot arrives
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[seaside] auto-apply failed; you can switch manually under Settings → Skin Market', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-seaside-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(SeasideStatusDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-seaside-dock-open')
  }
}
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[seaside] brand slot %s takeover failed; keeping the official mark', slot.name, error)
        return () => {}
      }
    }))
  }
