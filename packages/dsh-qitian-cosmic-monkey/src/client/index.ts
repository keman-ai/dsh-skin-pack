...
import { QitianMark, QitianName } from './Brand.tsx'
import { QitianStatusDock } from './StatusDock.tsx'
import { QitianEnergyRail } from './EnergyRail.tsx'
import { QitianStatusProbe } from './StatusProbe.tsx'
import { QITIAN_TOKENS } from './tokens.ts'
import './qitian.module.css'

export { QITIAN_PALETTE, QITIAN_TOKENS } from './tokens.ts'

/** Theme id, and the argument to `setTheme`. Three ids must agree: this constant, skin.json and cordis.patch.yml. */
export const THEME_ID = 'qitian'

/** Body marker: the single hook for the decorative CSS, and a convenient handle for user overrides. */
export const BODY_ATTRIBUTE = 'data-dsh-qitian'
...
const COVER_VARIABLE = '--qitian-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-qitian-dock-open'
const DOCK_STORAGE_KEY = 'qitian'

/**
 * The cover URL, served by the host half at `/skin-cover/qitian.webp` (see COVER_ROUTE in src/index.ts).
 *
 ...
 */
const COVER_URL = '/skin-cover/qitian.webp?v=0.1.1'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: QitianMark },
  { name: 'sidebar.brand.name', component: QitianName },
  { name: 'conversation.hero.brand.mark', component: QitianMark },
] as const
...
export interface Config {
  /**
   * Switch to Qitian Starscape on install; on by default.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * Mounting does not depend on whether the skin is active; visibility is left to CSS (display only under `body[data-dsh-qitian]`) —
   * The rule is "not active means not present", so no half-built UI shows during the window before the skin takes effect.
   */
  ctx.effect(() => mountDock(), 'qitian: status dock')
  ...
  ctx.effect(() => ctx.slots.inject('conversation.composer.dock', () => ctx.slots.register({
    ...
    order: 100,
  }, QitianStatusProbe)), 'qitian: status probe')
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    id: 'qitian-energy',
    order: 100,
  }, QitianEnergyRail)), 'qitian: energy rail')
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'light', tokens: QITIAN_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'qitian: theme + cover + brand')
}
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[qitian] another skin already claimed the auto-apply slot (%s); this one waits to be picked', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   * (`isThemePreference('qitian')` is false and third-party ids never reach persistence), and when the Host snapshot
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[qitian] auto-apply failed; you can switch manually under Settings → Skin Market', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-qitian-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(QitianStatusDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-qitian-dock-open')
  }
}
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[qitian] brand slot %s takeover failed; keeping the official mark', slot.name, error)
        return () => {}
      }
    }))
  }
