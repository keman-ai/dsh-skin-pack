/**
 * Black Myth Wukong · Flame Mountain · browser half.
 *
 ...
 * 2. **Mount the hero visual** — stamp our own attribute on body and hand the Flame Mountain image to the stylesheet as a CSS variable. Only our own attributes and
...
import { WukongMark, WukongName } from './Brand.tsx'
import { WukongStatusDock } from './StatusDock.tsx'
import { WukongEnergyRail } from './EnergyRail.tsx'
import { WukongStatusProbe } from './StatusProbe.tsx'
import { WUKONG_TOKENS } from './tokens.ts'
import './wukong.module.css'

export { WUKONG_PALETTE, WUKONG_TOKENS } from './tokens.ts'

/** Theme id, and the argument to `setTheme`. Three ids must agree: this constant, skin.json and cordis.patch.yml. */
export const THEME_ID = 'wukong'

/** Body marker: the single hook for the decorative CSS, and a convenient handle for user overrides. */
export const BODY_ATTRIBUTE = 'data-dsh-wukong'
...
const COVER_VARIABLE = '--wukong-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-wukong-dock-open'
const DOCK_STORAGE_KEY = 'wukong'

/**
 * The cover URL, served by the host half at `/skin-cover/wukong.webp` (see COVER_ROUTE in src/index.ts).
 *
 ...
 */
const COVER_URL = '/skin-cover/wukong.webp?v=0.1.0'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: WukongMark },
  { name: 'sidebar.brand.name', component: WukongName },
  { name: 'conversation.hero.brand.mark', component: WukongMark },
] as const
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * Mounting does not depend on whether the skin is active; visibility is left to CSS (display only under `body[data-dsh-wukong]`) —
   * The rule is "not active means not present", so no half-built UI shows during the window before the skin takes effect.
   */
  ctx.effect(() => mountDock(), 'wukong: status dock')
  ...
  ctx.effect(() => ctx.slots.inject('conversation.composer.dock', () => ctx.slots.register({
    ...
    order: 100,
  }, WukongStatusProbe)), 'wukong: status probe')
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    id: 'wukong-energy',
    order: 100,
  }, WukongEnergyRail)), 'wukong: energy rail')
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'dark', tokens: WUKONG_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'wukong: theme + cover + brand')
}
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[wukong] another skin already claimed the auto-apply slot (%s); this one waits to be picked', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   * (`isThemePreference('wukong')` is false and third-party ids never reach persistence), and when the Host snapshot arrives
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[wukong] auto-apply failed; you can switch manually under Settings → Skin Market', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-wukong-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(WukongStatusDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-wukong-dock-open')
  }
}
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[wukong] brand slot %s takeover failed; keeping the official mark', slot.name, error)
        return () => {}
      }
    }))
  }
