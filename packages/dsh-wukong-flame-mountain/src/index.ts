 ...
 *
 * @module dsh-wukong-flame-mountain
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { WUKONG_COVER } from './cover.generated.ts'

/** Plugin name (the `name` of the loader entry). */
export const name = 'wukong'
 ...
 */
export const COVER_ROUTE = '/skin-cover/wukong.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(WUKONG_COVER.slice(WUKONG_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Flame Mountain on install; on by default. Turned off, it only registers without applying, leaving the user to pick it in the skin market.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.logger.info('[wukong] Black Myth Wukong · Flame Mountain mounted (%s)', mode)
}
