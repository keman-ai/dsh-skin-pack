...
import { TwinwhaleMark, TwinwhaleName } from './Brand.tsx'
import { TwinwhaleStatusDock } from './StatusDock.tsx'
import { FishEnergyRail } from './EnergyRail.tsx'
import { TwinwhaleStatusProbe } from './StatusProbe.tsx'
import { TWINWHALE_TOKENS } from './tokens.ts'
import './twinwhale.module.css'

export { TWINWHALE_PALETTE, TWINWHALE_TOKENS } from './tokens.ts'
...
export const BODY_ATTRIBUTE = 'data-dsh-twinwhale'
...
const COVER_VARIABLE = '--twinwhale-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-twinwhale-dock-open'
const DOCK_STORAGE_KEY = 'twinwhale'

/**
 * The cover URL, served by the host half at `/skin-cover/twinwhale.webp` (see COVER_ROUTE in src/index.ts).
 *
 ...
 */
const COVER_URL = '/skin-cover/twinwhale.webp?v=0.1.0'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: TwinwhaleMark },
  { name: 'sidebar.brand.name', component: TwinwhaleName },
  { name: 'conversation.hero.brand.mark', component: TwinwhaleMark },
] as const
...
export interface Config {
  /**
   * Switch to Twin Whale Girl on install; on by default.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * Mounting does not depend on whether the skin is active; visibility is left to CSS (display only under `body[data-dsh-twinwhale]`) —
   * The rule is "not active means not present", so no half-built UI shows during the window before the skin takes effect.
   */
  ctx.effect(() => mountDock(), 'twinwhale: status dock')
  ...
  ctx.effect(() => ctx.slots.inject('conversation.composer.dock', () => ctx.slots.register({
    ...
    order: 100,
  }, TwinwhaleStatusProbe)), 'twinwhale: status probe')
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    id: 'twinwhale-energy',
    order: 100,
  }, FishEnergyRail)), 'twinwhale: energy rail')
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'light', tokens: TWINWHALE_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'twinwhale: theme + cover + brand')
}
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[twinwhale] another skin already claimed the auto-apply slot (%s); this one waits to be picked', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   * (`isThemePreference('twinwhale')` is false and third-party ids never reach persistence), and when the Host snapshot
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[twinwhale] auto-apply failed; you can switch manually under Settings → Skin Market', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-twinwhale-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(TwinwhaleStatusDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-twinwhale-dock-open')
  }
}
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[twinwhale] brand slot %s takeover failed; keeping the official mark', slot.name, error)
        return () => {}
      }
    }))
  }
