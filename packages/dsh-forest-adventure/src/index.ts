/**
 * The Forest Adventure skin · host half.
 *
 ...
 *
 * @module dsh-forest-adventure
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { GROVE_COVER } from './cover.generated.ts'

/** Plugin name (the `name` of the loader entry). */
export const name = 'grove'
 ...
 */
export const COVER_ROUTE = '/skin-cover/grove.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(GROVE_COVER.slice(GROVE_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Forest on install; on by default. Turned off, it only registers without applying, leaving the user to pick it under Settings → Appearance.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.logger.info('[grove] Forest Adventure mounted (%s)', mode)
}
