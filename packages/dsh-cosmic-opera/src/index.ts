/**
 * The Cosmic Opera skin · host half.
 *
 ...
 *
 * @module dsh-cosmic-opera
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { OPERA_COVER } from './cover.generated.ts'
 ...
 */
export const COVER_ROUTE = '/skin-cover/opera.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(OPERA_COVER.slice(OPERA_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Cosmic Opera on install; on by default. Turned off, it only registers without applying, leaving the user to pick it in the skin market.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.logger.info('[opera] Cosmic Opera mounted (%s)', mode)
}
