...
import { OperaMark, OperaName } from './Brand.tsx'
import { OperaStatusDock } from './StatusDock.tsx'
import { OperaEnergyRail } from './EnergyRail.tsx'
import { OperaStatusProbe } from './StatusProbe.tsx'
import { OPERA_TOKENS } from './tokens.ts'
import './opera.module.css'

export { OPERA_PALETTE, OPERA_TOKENS } from './tokens.ts'

/** Theme id, and the argument to `setTheme`. Three ids must agree: this constant, skin.json and cordis.patch.yml. */
export const THEME_ID = 'opera'

/** Body marker: the single hook for the decorative CSS, and a convenient handle for user overrides. */
export const BODY_ATTRIBUTE = 'data-dsh-opera'
...
const COVER_VARIABLE = '--opera-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-opera-dock-open'
 ...
 */
const COVER_URL = '/skin-cover/opera.webp?v=0.1.0'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: OperaMark },
  { name: 'sidebar.brand.name', component: OperaName },
  { name: 'conversation.hero.brand.mark', component: OperaMark },
] as const
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.effect(() => ctx.slots.inject('conversation.composer.dock', () => ctx.slots.register({
    ...
    order: 100,
  }, OperaStatusProbe)), 'opera: status probe')
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    id: 'opera-energy',
    order: 100,
  }, OperaEnergyRail)), 'opera: energy rail')
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'dark', tokens: OPERA_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'opera: theme + cover + brand')
}
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-opera-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(OperaStatusDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-opera-dock-open')
  }
}
