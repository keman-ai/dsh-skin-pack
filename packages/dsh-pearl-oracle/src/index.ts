/**
 * The Pearl Oracle skin · host half.
 *
 ...
 *
 * @module dsh-pearl-oracle
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { PEARL_COVER } from './cover.generated.ts'
 ...
 */
export const COVER_ROUTE = '/skin-cover/pearl.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(PEARL_COVER.slice(PEARL_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Pearl Oracle on install; on by default. Turned off, it only registers without applying, leaving the user to pick it under Settings → Appearance.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.logger.info('[pearl] Pearl Oracle mounted (%s)', mode)
}
