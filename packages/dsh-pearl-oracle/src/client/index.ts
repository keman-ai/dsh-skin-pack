...
/**
 * 封面地址。由 host 半在 `/skin-cover/pearl.webp` 上提供（见 src/index.ts 的 COVER_ROUTE）。
 *
 * 🔴 不再内联 data URI：多套皮肤同时装载时，每套几百 KB 的 base64 会把浏览器主线程压死
 *（实测 21 套同装时首屏 90 秒渲染不出来）。现在浏览器只在皮肤**真的激活**、CSS 用到这个
 * 变量时才去取图。
 */
const COVER_URL = '/skin-cover/pearl.webp'
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * 挂载不区分皮肤是否激活，可见性交给 CSS（`body[data-dsh-pearl]` 才 display）——
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'light', tokens: PEARL_TOKENS })
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[pearl] 已有皮肤占了自动应用名额（%s），本套改为待选', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   *（`isThemePreference('pearl')` 是 false，第三方 id 根本不进持久化），而 Host 快照
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[pearl] 自动应用失败，可到「设置 → 皮肤市场」手动切换', error)
      }
      return
    }
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[pearl] 品牌位 %s 接管失败，保留官方标', slot.name, error)
        return () => {}
      }
    }))
  }
