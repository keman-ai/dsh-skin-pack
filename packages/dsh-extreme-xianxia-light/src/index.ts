/**
 * The Misty Xianxia skin · host half.
 *
 ...
 *
 * @module dsh-extreme-xianxia-light
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { MIST_COVER } from './cover.generated.ts'

/** Plugin name (the `name` of the loader entry). */
export const name = 'mist'
 ...
 */
export const COVER_ROUTE = '/skin-cover/mist.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(MIST_COVER.slice(MIST_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Misty Xianxia on install; on by default. Turned off, it only registers without applying, leaving the user to pick it in the skin market.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.logger.info('[mist] Misty Xianxia mounted (%s)', mode)
}
