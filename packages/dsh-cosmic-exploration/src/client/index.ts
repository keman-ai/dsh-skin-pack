/**
 * Cosmic Exploration · browser half.
 *
...
import { CosmicMark, CosmicName } from './Brand.tsx'
import { CosmicStatusDock } from './StatusDock.tsx'
import { ExplorationEnergyRail } from './EnergyRail.tsx'
import { CosmicStatusProbe } from './StatusProbe.tsx'
import { COSMIC_TOKENS } from './tokens.ts'
import './cosmic.module.css'

export { COSMIC_PALETTE, COSMIC_TOKENS } from './tokens.ts'

/** Theme id, and the argument to `setTheme`. Three ids must agree: this constant, skin.json and cordis.patch.yml. */
export const THEME_ID = 'cosmic'

/** Body marker: the single hook for the decorative CSS, and a convenient handle for user overrides. */
export const BODY_ATTRIBUTE = 'data-dsh-cosmic'
...
const COVER_VARIABLE = '--cosmic-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-cosmic-dock-open'
const DOCK_STORAGE_KEY = 'cosmic'

/**
 * The cover URL, served by the host half at `/skin-cover/cosmic.webp` (see COVER_ROUTE in src/index.ts).
 *
 ...
 */
const COVER_URL = '/skin-cover/cosmic.webp?v=0.1.1'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: CosmicMark },
  { name: 'sidebar.brand.name', component: CosmicName },
  { name: 'conversation.hero.brand.mark', component: CosmicMark },
] as const
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * Mounting does not depend on whether the skin is active; visibility is left to CSS (display only under `body[data-dsh-cosmic]`) —
   * The rule is "not active means not present", so no half-built UI shows during the window before the skin takes effect.
   */
  ctx.effect(() => mountDock(), 'cosmic: status dock')
  ...
  ctx.effect(() => ctx.slots.inject('conversation.composer.dock', () => ctx.slots.register({
    ...
    order: 100,
  }, CosmicStatusProbe)), 'cosmic: status probe')
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    id: 'exploration-energy',
    order: 100,
  }, ExplorationEnergyRail)), 'exploration: energy rail')
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'dark', tokens: COSMIC_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'cosmic: theme + cover + brand')
}
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[cosmic] another skin already claimed the auto-apply slot (%s); this one waits to be picked', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   * (`isThemePreference('cosmic')` is false and third-party ids never reach persistence), and when the Host snapshot arrives
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[cosmic] auto-apply failed; you can switch manually under Settings → Skin Market', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-cosmic-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(CosmicStatusDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-cosmic-dock-open')
  }
}
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[cosmic] brand slot %s takeover failed; keeping the official mark', slot.name, error)
        return () => {}
      }
    }))
  }
