 ...
 * 旁边两行「DeepSeek Harness / 双鲸女仆 Theme」。
 *
 ...
 * @returns 方形鲸鱼标。
 */
export function TwinmaidMark({ size, className }: BrandMarkProps) {
  return (
    <span
...
/**
 * 站名：主名 + 副标，对应原型稿的「DeepSeek Harness / 双鲸女仆 Theme」。
 *
 * 主名保留 DeepSeek Harness —— 皮肤换的是外观，不冒充另一个产品；副标才是这套皮肤的身份。
 *
 * @returns 两行站名。
 */
export function TwinmaidName() {
  return (
    <span className={css.name}>
      <strong className={css.title}>DeepSeek Harness</strong>
      <small className={css.subtitle}>双鲸女仆</small>
    </span>
  )
}
