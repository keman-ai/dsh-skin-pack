...
import { TwinmaidMark, TwinmaidName } from './Brand.tsx'
import { TwinmaidStatusDock } from './StatusDock.tsx'
import { FishEnergyRail } from './EnergyRail.tsx'
import { TwinmaidStatusProbe } from './StatusProbe.tsx'
import { TWINMAID_TOKENS } from './tokens.ts'
import './twinmaid.module.css'

export { TWINMAID_PALETTE, TWINMAID_TOKENS } from './tokens.ts'

/** 主题 id，也是 `setTheme` 的参数。三处 id 必须一致：本常量、skin.json、cordis.patch.yml。 */
export const THEME_ID = 'twinmaid'

/** body 标记：装饰 CSS 的唯一挂点，同时便于用户自写覆盖规则。 */
export const BODY_ATTRIBUTE = 'data-dsh-twinmaid'

/** 封面变量名：CSS 里读它，值在这里注入，图片资源不进样式表（否则样式表被撑爆且不好清）。 */
const COVER_VARIABLE = '--twinmaid-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-twinmaid-dock-open'
const DOCK_STORAGE_KEY = 'twinmaid'

/**
 * 封面地址。由 host 半在 `/skin-cover/twinmaid.webp` 上提供（见 src/index.ts 的 COVER_ROUTE）。
 *
 * 🔴 不再内联 data URI：多套皮肤同时装载时，每套几百 KB 的 base64 会把浏览器主线程压死
 *（实测 21 套同装时首屏 90 秒渲染不出来）。现在浏览器只在皮肤**真的激活**、CSS 用到这个
 * 变量时才去取图。
 */
const COVER_URL = '/skin-cover/twinmaid.webp'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: TwinmaidMark },
  { name: 'sidebar.brand.name', component: TwinmaidName },
  { name: 'conversation.hero.brand.mark', component: TwinmaidMark },
] as const
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * 挂载不区分皮肤是否激活，可见性交给 CSS（`body[data-dsh-twinmaid]` 才 display）——
   * 判据写成"没激活 = 不存在"，避免皮肤生效前那段窗口里露出半成品界面。
   */
  ctx.effect(() => mountDock(), 'twinmaid: status dock')
  ...
  ctx.effect(() => ctx.slots.inject('conversation.composer.dock', () => ctx.slots.register({
    ...
    order: 100,
  }, TwinmaidStatusProbe)), 'twinmaid: status probe')
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[twinmaid] 已有皮肤占了自动应用名额（%s），本套改为待选', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   *（`isThemePreference('twinmaid')` 是 false，第三方 id 根本不进持久化），而 Host 快照
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[twinmaid] 自动应用失败，可到「设置 → 皮肤市场」手动切换', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-twinmaid-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(TwinmaidStatusDock))
  return () => {
    // 异步卸载：React 不允许在自己的渲染周期内同步 unmount。
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-twinmaid-dock-open')
  }
}
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[twinmaid] 品牌位 %s 接管失败，保留官方标', slot.name, error)
        return () => {}
      }
    }))
  }
