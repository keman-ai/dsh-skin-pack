...
import { CatbusMark, CatbusName } from './Brand.tsx'
import { CatbusStatusDock } from './StatusDock.tsx'
import { CatbusEnergyRail } from './EnergyRail.tsx'
import { CatbusStatusProbe } from './StatusProbe.tsx'
import { CATBUS_TOKENS } from './tokens.ts'
import './catbus.module.css'

export { CATBUS_PALETTE, CATBUS_TOKENS } from './tokens.ts'

/** Theme id, and the argument to `setTheme`. Three ids must agree: this constant, skin.json and cordis.patch.yml. */
export const THEME_ID = 'catbus'

/** Body marker: the single hook for the decorative CSS, and a convenient handle for user overrides. */
export const BODY_ATTRIBUTE = 'data-dsh-catbus'
...
const COVER_VARIABLE = '--catbus-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-catbus-dock-open'
 ...
 */
const COVER_URL = '/skin-cover/catbus.webp?v=0.1.0'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: CatbusMark },
  { name: 'sidebar.brand.name', component: CatbusName },
  { name: 'conversation.hero.brand.mark', component: CatbusMark },
] as const
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.effect(() => ctx.slots.inject('conversation.composer.dock', () => ctx.slots.register({
    ...
    order: 100,
  }, CatbusStatusProbe)), 'catbus: status probe')
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    id: 'catbus-energy',
    order: 100,
  }, CatbusEnergyRail)), 'catbus: energy rail')
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'light', tokens: CATBUS_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'catbus: theme + cover + brand')
}
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-catbus-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(CatbusStatusDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-catbus-dock-open')
  }
}
