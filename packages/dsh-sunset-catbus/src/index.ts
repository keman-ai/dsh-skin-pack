/**
 * The Sunset Catbus skin · host half.
 *
 ...
 *
 * @module dsh-sunset-catbus
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { CATBUS_COVER } from './cover.generated.ts'

/** Plugin name (the `name` of the loader entry). */
export const name = 'catbus'
 ...
 */
export const COVER_ROUTE = '/skin-cover/catbus.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(CATBUS_COVER.slice(CATBUS_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Sunset Catbus on install; on by default. Turned off, it only registers without applying, leaving the user to pick it under Settings → Appearance.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.logger.info('[catbus] Sunset Catbus mounted (%s)', mode)
}
