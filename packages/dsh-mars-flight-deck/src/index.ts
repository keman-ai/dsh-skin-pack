/**
 * The Mars Flight Deck skin · host half.
 *
 ...
 *
 * @module dsh-mars-flight-deck
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { MARS_COVER } from './cover.generated.ts'

/** Plugin name (the `name` of the loader entry). */
export const name = 'mars'
 ...
 */
export const COVER_ROUTE = '/skin-cover/mars.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(MARS_COVER.slice(MARS_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Mars Flight Deck on install; on by default. Turned off, it only registers without applying, leaving the user to pick it in the skin market.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.logger.info('[mars] Mars Flight Deck mounted (%s)', mode)
}
