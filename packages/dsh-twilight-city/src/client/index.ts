...
import { TwilightMark, TwilightName } from './Brand.tsx'
import { TwilightStatusDock } from './StatusDock.tsx'
import { TwilightEnergyRail } from './EnergyRail.tsx'
import { TwilightStatusProbe } from './StatusProbe.tsx'
import { TWILIGHT_TOKENS } from './tokens.ts'
import './twilight.module.css'

export { TWILIGHT_PALETTE, TWILIGHT_TOKENS } from './tokens.ts'
...
/**
 * The cover URL, served by the host half at `/skin-cover/twilight.webp` (see COVER_ROUTE in src/index.ts).
 *
 ...
 */
const COVER_URL = '/skin-cover/twilight.webp?v=0.1.0'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: TwilightMark },
  { name: 'sidebar.brand.name', component: TwilightName },
  { name: 'conversation.hero.brand.mark', component: TwilightMark },
] as const
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * Mounting does not depend on whether the skin is active; visibility is left to CSS (display only under `body[data-dsh-twilight]`) —
   * The rule is "not active means not present", so no half-built UI shows during the window before the skin takes effect.
   */
  ctx.effect(() => mountDock(), 'twilight: status dock')
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[twilight] another skin already claimed the auto-apply slot (%s); this one waits to be picked', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   * (`isThemePreference('twilight')` is false and third-party ids never reach persistence), and when the Host snapshot arrives
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[twilight] auto-apply failed; you can switch manually under Settings → Skin Market', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-twilight-dock', '')
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[twilight] brand slot %s takeover failed; keeping the official mark', slot.name, error)
        return () => {}
      }
    }))
  }
