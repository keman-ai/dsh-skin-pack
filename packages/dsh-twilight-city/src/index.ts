/**
 * The Twilight City skin · host half.
 *
 ...
 *
 * @module dsh-twilight-city
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { TWILIGHT_COVER } from './cover.generated.ts'

/** Plugin name (the `name` of the loader entry). */
export const name = 'twilight'
 ...
 */
export const COVER_ROUTE = '/skin-cover/twilight.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(TWILIGHT_COVER.slice(TWILIGHT_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Twilight City on install; on by default. Turned off, it only registers without applying, leaving the user to pick it in the skin market.
   *
