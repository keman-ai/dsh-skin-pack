...
import { HillsideMark, HillsideName } from './Brand.tsx'
import { HillsideStatusDock } from './StatusDock.tsx'
import { MistEnergyRail } from './EnergyRail.tsx'
import { HillsideStatusProbe } from './StatusProbe.tsx'
import { HILLSIDE_TOKENS } from './tokens.ts'
import './hillside.module.css'

export { HILLSIDE_PALETTE, HILLSIDE_TOKENS } from './tokens.ts'
...
export const BODY_ATTRIBUTE = 'data-dsh-hillside'
...
const COVER_VARIABLE = '--hillside-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-hillside-dock-open'
const DOCK_STORAGE_KEY = 'hillside'

/**
 * The cover URL, served by the host half at `/skin-cover/hillside.webp` (see COVER_ROUTE in src/index.ts).
 *
 ...
 */
const COVER_URL = '/skin-cover/hillside.webp?v=0.1.0'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: HillsideMark },
  { name: 'sidebar.brand.name', component: HillsideName },
  { name: 'conversation.hero.brand.mark', component: HillsideMark },
] as const
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * Mounting does not depend on whether the skin is active; visibility is left to CSS (display only under `body[data-dsh-hillside]`) —
   * The rule is "not active means not present", so no half-built UI shows during the window before the skin takes effect.
   */
  ctx.effect(() => mountDock(), 'hillside: status dock')
  ...
  ctx.effect(() => ctx.slots.inject('conversation.composer.dock', () => ctx.slots.register({
    ...
    order: 100,
  }, HillsideStatusProbe)), 'hillside: status probe')
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    id: 'hillside-energy',
    order: 100,
  }, MistEnergyRail)), 'hillside: energy rail')
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'light', tokens: HILLSIDE_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'hillside: theme + cover + brand')
}
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[hillside] another skin already claimed the auto-apply slot (%s); this one waits to be picked', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   * (`isThemePreference('hillside')` is false and third-party ids never reach persistence), and when the Host snapshot arrives
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[hillside] auto-apply failed; you can switch manually under Settings → Skin Market', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-hillside-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(HillsideStatusDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-hillside-dock-open')
  }
}
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[hillside] brand slot %s takeover failed; keeping the official mark', slot.name, error)
        return () => {}
      }
    }))
  }
