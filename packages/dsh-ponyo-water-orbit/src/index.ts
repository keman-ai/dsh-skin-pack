/**
 * The Ponyo Water Orbit skin · host half.
 *
 ...
 *
 * @module dsh-ponyo-water-orbit
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { PONYO_COVER } from './cover.generated.ts'

/** Plugin name (the `name` of the loader entry). */
export const name = 'ponyo'
 ...
 */
export const COVER_ROUTE = '/skin-cover/ponyo.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(PONYO_COVER.slice(PONYO_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Ponyo Water Orbit on install; on by default. Turned off, it only registers without applying, leaving the user to pick it under Settings → Appearance.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.logger.info('[ponyo] Ponyo Water Orbit mounted (%s)', mode)
}
