 ...
 *
 * @module dsh-deepseek-twin-whale-maid
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { TWINMAID_COVER } from './cover.generated.ts'

/** 插件名（loader 行的 name）。 */
export const name = 'twinmaid'
 ...
 */
export const COVER_ROUTE = '/skin-cover/twinmaid.webp'

/** 封面字节。data URI 只在这一侧解一次，之后常驻内存（几百 KB，Node 侧无所谓）。 */
const COVER_BYTES = Buffer.from(TWINMAID_COVER.slice(TWINMAID_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * 装上就切到双鲸女仆，默认开。关掉则只注册、不应用，等用户自己去「设置 → 外观」选。
   *
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  }), `twinmaid: ${COVER_ROUTE}`)

  const mode = config.autoApply === false ? '需在「设置 → 外观」里手动选' : '已自动应用'
  ctx.logger.info('[twinmaid] 双鲸女仆已挂载（%s）', mode)
}
