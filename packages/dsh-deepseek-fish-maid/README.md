✔ dsh-deepseek-fish-maid/README.md
 Deep Sea
...
- **新会话页整幅封面**：鱼娘、海底遗迹、鲸鱼全息屏与气泡。进入对话与轨迹页后封面收起。
- **品牌标接管**：侧栏与 hero 的标都换成一枚鲸鱼方标（内联 svg，不是 emoji），副标「大肥鱼娘 · Deep Sea」。
- **人格化文案**：思考中 → 「正在潜入上下文……」；失败 → 「好像撞到暗礁了，请重试。」；
  需要你确认时前缀一句「大肥鱼娘需要你的确认后才能继续」——**原文照旧留着**，那才是你做判断的依据。
- **右侧状态台**：常驻一根，见下表。
...
DST=~/.dsh/profiles/web/node_modules/dsh-deepseek-fish-maid
mkdir -p "$DST" && cp -R lib cordis.patch.yml skin.json package.json README.md "$DST/"
# 再把 dsh-deepseek-fish-maid 加进 profile 的 package.json 的 dependencies 与 dsh.profile.bundles
```
