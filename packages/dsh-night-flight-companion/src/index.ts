/**
 * The Night Flight Companion skin · host half.
 *
 ...
 * the tree, the Loader imports this file, and the browser half is loaded from `package.json`'s `dsh.client`
 * declaration. An empty plugin is clearer than none — without it that Loader row points at a package with no entry (and the market's install-time check would report `BUILD_SCRIPT_BLOCKED` and uninstall it).
 *
 * @module dsh-night-flight-companion
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { NIGHT_COVER } from './cover.generated.ts'

/** Plugin name (the `name` of the loader entry). */
export const name = 'night-flight'
 ...
 */
export const COVER_ROUTE = '/skin-cover/night-flight.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(NIGHT_COVER.slice(NIGHT_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Night Flight on install; on by default. Turned off, it only registers without applying, leaving the user to pick it under Settings → Appearance.
   *
