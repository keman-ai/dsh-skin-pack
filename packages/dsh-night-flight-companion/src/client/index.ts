/**
 * Night Flight Companion · browser half.
 *
...
import { NightMark, NightName } from './Brand.tsx'
import { NightStatusDock } from './StatusDock.tsx'
import { NightEnergyRail } from './EnergyRail.tsx'
import { NightStatusProbe } from './StatusProbe.tsx'
import { NIGHT_TOKENS } from './tokens.ts'
import './night.module.css'

export { NIGHT_PALETTE, NIGHT_TOKENS } from './tokens.ts'

/** Theme id, and the argument to `setTheme`. Three ids must agree: this constant, skin.json and cordis.patch.yml. */
export const THEME_ID = 'night-flight'

/** Body marker: the single hook for the decorative CSS, and a convenient handle for user overrides. */
export const BODY_ATTRIBUTE = 'data-dsh-night-flight'
...
const COVER_VARIABLE = '--night-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-night-flight-dock-open'
const DOCK_STORAGE_KEY = 'night-flight'

/**
 * The cover URL, served by the host half at `/skin-cover/night-flight.webp` (see COVER_ROUTE in src/index.ts).
 *
 ...
 */
const COVER_URL = '/skin-cover/night-flight.webp?v=0.1.1'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: NightMark },
  { name: 'sidebar.brand.name', component: NightName },
  { name: 'conversation.hero.brand.mark', component: NightMark },
] as const
...
export interface Config {
  /**
   * Switch to Night Flight on install; on by default.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * Mounting does not depend on whether the skin is active; visibility is left to CSS (display only under `body[data-dsh-night-flight]`) —
   * The rule is "not active means not present", so no half-built UI shows during the window before the skin takes effect.
   */
  ctx.effect(() => mountDock(), 'night-flight: status dock')
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    id: 'night-energy',
    order: 100,
  }, NightEnergyRail)), 'night: energy rail')
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'light', tokens: NIGHT_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'night-flight: theme + cover + brand')
}
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[night-flight] another skin already claimed the auto-apply slot (%s); this one waits to be picked', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   * (`isThemePreference('night-flight')` is false and third-party ids never reach persistence), and when the Host snapshot
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[night-flight] auto-apply failed; you can switch manually under Settings → Skin Market', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-night-flight-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(NightStatusDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-night-flight-dock-open')
  }
}
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[night-flight] brand slot %s takeover failed; keeping the official mark', slot.name, error)
        return () => {}
      }
    }))
  }
