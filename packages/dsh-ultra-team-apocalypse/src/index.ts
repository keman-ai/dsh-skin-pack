/**
 * The Ultra Team Apocalypse skin · host half.
 *
 ...
 *
 * @module dsh-ultra-team-apocalypse
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { ULTRATEAM_COVER } from './cover.generated.ts'

/** Plugin name (the `name` of the loader entry). */
export const name = 'ultrateam'
 ...
 */
export const COVER_ROUTE = '/skin-cover/ultrateam.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(ULTRATEAM_COVER.slice(ULTRATEAM_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Ultra Team Apocalypse on install; on by default. Turned off, it only registers without applying, leaving the user to pick it in the skin market.
   *
