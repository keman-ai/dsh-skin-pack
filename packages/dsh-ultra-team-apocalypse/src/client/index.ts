/**
 * Ultra Team · Apocalypse · browser half.
 *
...
import { UltrateamMark, UltrateamName } from './Brand.tsx'
import { UltrateamStatusDock } from './StatusDock.tsx'
import { UltrateamEnergyRail } from './EnergyRail.tsx'
import { UltrateamStatusProbe } from './StatusProbe.tsx'
import { ULTRATEAM_TOKENS } from './tokens.ts'
import './ultrateam.module.css'

export { ULTRATEAM_PALETTE, ULTRATEAM_TOKENS } from './tokens.ts'

/** Theme id, and the argument to `setTheme`. Three ids must agree: this constant, skin.json and cordis.patch.yml. */
export const THEME_ID = 'ultrateam'

/** Body marker: the single hook for the decorative CSS, and a convenient handle for user overrides. */
export const BODY_ATTRIBUTE = 'data-dsh-ultrateam'
...
const COVER_VARIABLE = '--ultrateam-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-ultrateam-dock-open'
const DOCK_STORAGE_KEY = 'ultrateam'

/**
 * The cover URL, served by the host half at `/skin-cover/ultrateam.webp` (see COVER_ROUTE in src/index.ts).
 *
 ...
 */
const COVER_URL = '/skin-cover/ultrateam.webp?v=0.1.0'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: UltrateamMark },
  { name: 'sidebar.brand.name', component: UltrateamName },
  { name: 'conversation.hero.brand.mark', component: UltrateamMark },
] as const
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * Mounting does not depend on whether the skin is active; visibility is left to CSS (display only under `body[data-dsh-ultrateam]`) —
   * The rule is "not active means not present", so no half-built UI shows during the window before the skin takes effect.
   */
  ctx.effect(() => mountDock(), 'ultrateam: status dock')
  ...
  ctx.effect(() => ctx.slots.inject('conversation.composer.dock', () => ctx.slots.register({
    ...
    order: 100,
  }, UltrateamStatusProbe)), 'ultrateam: status probe')
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    id: 'ultrateam-energy',
    order: 100,
  }, UltrateamEnergyRail)), 'ultrateam: energy rail')
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'dark', tokens: ULTRATEAM_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'ultrateam: theme + cover + brand')
}
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[ultrateam] another skin already claimed the auto-apply slot (%s); this one waits to be picked', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   * (`isThemePreference('ultrateam')` is false and third-party ids never reach persistence), and when the Host snapshot arrives
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[ultrateam] auto-apply failed; you can switch manually under Settings → Skin Market', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-ultrateam-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(UltrateamStatusDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-ultrateam-dock-open')
  }
}
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[ultrateam] brand slot %s takeover failed; keeping the official mark', slot.name, error)
        return () => {}
      }
    }))
  }
