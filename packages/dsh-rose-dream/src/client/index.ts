...
import { RoseMark, RoseName } from './Brand.tsx'
import { RoseStatusDock } from './StatusDock.tsx'
import { MistEnergyRail } from './EnergyRail.tsx'
import { RoseStatusProbe } from './StatusProbe.tsx'
import { ROSE_TOKENS } from './tokens.ts'
import './rose.module.css'

export { ROSE_PALETTE, ROSE_TOKENS } from './tokens.ts'
...
export const BODY_ATTRIBUTE = 'data-dsh-rose'
...
const COVER_VARIABLE = '--rose-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-rose-dock-open'
const DOCK_STORAGE_KEY = 'rose'

/**
 * The cover URL, served by the host half at `/skin-cover/rose.webp` (see COVER_ROUTE in src/index.ts).
 *
 ...
 */
const COVER_URL = '/skin-cover/rose.webp?v=0.1.0'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: RoseMark },
  { name: 'sidebar.brand.name', component: RoseName },
  { name: 'conversation.hero.brand.mark', component: RoseMark },
] as const
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * Mounting does not depend on whether the skin is active; visibility is left to CSS (display only under `body[data-dsh-rose]`) —
   * The rule is "not active means not present", so no half-built UI shows during the window before the skin takes effect.
   */
  ctx.effect(() => mountDock(), 'rose: status dock')
  ...
  ctx.effect(() => ctx.slots.inject('conversation.composer.dock', () => ctx.slots.register({
    ...
    order: 100,
  }, RoseStatusProbe)), 'rose: status probe')
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    id: 'rose-energy',
    order: 100,
  }, MistEnergyRail)), 'rose: energy rail')
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'light', tokens: ROSE_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'rose: theme + cover + brand')
}
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[rose] another skin already claimed the auto-apply slot (%s); this one waits to be picked', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   * (`isThemePreference('rose')` is false and third-party ids never reach persistence), and when the Host snapshot arrives
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[rose] auto-apply failed; you can switch manually under Settings → Skin Market', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-rose-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(RoseStatusDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-rose-dock-open')
  }
}
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[rose] brand slot %s takeover failed; keeping the official mark', slot.name, error)
        return () => {}
      }
    }))
  }
