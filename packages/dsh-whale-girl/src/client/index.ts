...
import { WhaleMark, WhaleName } from './Brand.tsx'
import { WhaleLoungeDock } from './LoungeDock.tsx'
import { WhaleStatusProbe } from './StatusProbe.tsx'
import { WHALE_TOKENS } from './tokens.ts'
import './whale.module.css'

export { WHALE_PALETTE, WHALE_TOKENS } from './tokens.ts'

/** Theme id, and the argument to `setTheme`. Three ids must agree: this constant, skin.json and cordis.patch.yml. */
export const THEME_ID = 'whale-girl'

/** Body marker: the single hook for the decorative CSS, and a convenient handle for user overrides. */
export const BODY_ATTRIBUTE = 'data-dsh-whale-girl'
...
const COVER_VARIABLE = '--whale-girl-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-whale-girl-dock-open'
 ...
 */
const COVER_URL = '/skin-cover/whale-girl.webp?v=0.1.1'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: WhaleMark },
  { name: 'sidebar.brand.name', component: WhaleName },
  { name: 'conversation.hero.brand.mark', component: WhaleMark },
] as const
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'light', tokens: WHALE_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'whale-girl: theme + cover + brand')
}
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-whale-girl-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(WhaleLoungeDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-whale-girl-dock-open')
  }
}
