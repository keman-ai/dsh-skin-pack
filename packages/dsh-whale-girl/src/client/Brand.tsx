...
export function WhaleMark({ size, className }: BrandMarkProps) {
  return (
    <svg
      ...
      focusable="false"
    >
      {/* The body: a whale rising to the right, with the forked fluke on the left. */}
      <path
        ...
        fill="currentColor"
      />
      {/* The eye: a hole rather than a dot, so it does not blur into a blob at 16px. */}
      <circle cx="22.4" cy="18.2" r="1.15" fill="var(--dsw-alias-bg-layer-2, #fff)" />
      {/* The spout: the whale girl's identifying feature, still recognisable at small sizes. */}
      <path
