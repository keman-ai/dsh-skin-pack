...
import { CosmicEnergyRail } from './EnergyRail.tsx'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-cosmic-hero-dock-open'
 ...
 */
const COVER_URL = '/skin-cover/cosmic-hero.webp?v=0.1.1'
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * Mounting does not depend on whether the skin is active; visibility is left to CSS (display only under `body[data-dsh-cosmic-hero]`) —
   * The rule is "not active means not present", so no half-built UI shows during the window before the skin takes effect.
   */
  ctx.effect(() => mountDock(), 'cosmic-hero: status dock')
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    id: 'cosmic-energy',
    order: 100,
  }, CosmicEnergyRail)), 'cosmic: energy rail')
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[cosmic-hero] another skin already claimed the auto-apply slot (%s); this one waits to be picked', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   * (`isThemePreference('cosmic-hero')` is false and third-party ids never reach persistence), and when the Host snapshot arrives
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[cosmic-hero] auto-apply failed; you can switch manually under Settings → Skin Market', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-cosmic-hero-dock', '')
  ...
  return () => {
    ...
    host.remove()
    document.body.removeAttribute('data-cosmic-hero-dock-open')
  }
}
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[cosmic-hero] brand slot %s takeover failed; keeping the official mark', slot.name, error)
        return () => {}
      }
    }))
  }
