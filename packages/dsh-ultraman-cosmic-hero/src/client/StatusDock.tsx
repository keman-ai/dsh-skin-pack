/**
 * The right-hand dock: a reduced cosmic hero visual plus this session's real state.
 *
...
export function CosmicStatusDock() {
  ...
  const core = occupancy === undefined
    ? { level: 'idle', label: '—' }
    : occupancy < 60 ? { level: 'stable', label: 'STABLE' }
      : occupancy < 85 ? { level: 'warn', label: 'CAUTION' }
            ...
            <section className={css.card}>
              ...
              {occupancy !== undefined && (
                <div className={css.progress}>
                  <span data-level={core.level} style={{ width: `${occupancy}%` }} />
                </div>
              )}
