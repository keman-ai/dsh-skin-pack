...
import { ForestMark, ForestName } from './Brand.tsx'
import { ForestStatusDock } from './StatusDock.tsx'
import { ForestEnergyRail } from './EnergyRail.tsx'
import { ForestStatusProbe } from './StatusProbe.tsx'
import { FOREST_TOKENS } from './tokens.ts'
import './forest.module.css'

export { FOREST_PALETTE, FOREST_TOKENS } from './tokens.ts'
...
export const BODY_ATTRIBUTE = 'data-dsh-forest'
...
const COVER_VARIABLE = '--forest-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-forest-dock-open'
const DOCK_STORAGE_KEY = 'forest'

/**
 * The cover URL, served by the host half at `/skin-cover/forest.webp` (see COVER_ROUTE in src/index.ts).
 *
 ...
 */
const COVER_URL = '/skin-cover/forest.webp?v=0.1.0'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: ForestMark },
  { name: 'sidebar.brand.name', component: ForestName },
  { name: 'conversation.hero.brand.mark', component: ForestMark },
] as const
...
export interface Config {
  /**
   * Switch to Forest on install; on by default.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * Mounting does not depend on whether the skin is active; visibility is left to CSS (display only under `body[data-dsh-forest]`) —
   * The rule is "not active means not present", so no half-built UI shows during the window before the skin takes effect.
   */
  ctx.effect(() => mountDock(), 'forest: status dock')
  ...
  ctx.effect(() => ctx.slots.inject('conversation.composer.dock', () => ctx.slots.register({
    ...
    order: 100,
  }, ForestStatusProbe)), 'forest: status probe')
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    id: 'forest-energy',
    order: 100,
  }, ForestEnergyRail)), 'forest: energy rail')
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'light', tokens: FOREST_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'forest: theme + cover + brand')
}
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[forest] another skin already claimed the auto-apply slot (%s); this one waits to be picked', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   * (`isThemePreference('forest')` is false and third-party ids never reach persistence), and when the Host snapshot
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[forest] auto-apply failed; you can switch manually under Settings → Skin Market', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-forest-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(ForestStatusDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-forest-dock-open')
  }
}
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[forest] brand slot %s takeover failed; keeping the official mark', slot.name, error)
        return () => {}
      }
    }))
  }
