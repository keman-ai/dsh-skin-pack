...
import { BwhaleMark, BwhaleName } from './Brand.tsx'
import { BwhaleStatusDock } from './StatusDock.tsx'
import { NforestEnergyRail } from './EnergyRail.tsx'
import { BwhaleStatusProbe } from './StatusProbe.tsx'
import { BWHALE_TOKENS } from './tokens.ts'
import './bwhale.module.css'
...
export const BODY_ATTRIBUTE = 'data-dsh-bwhale'
...
const COVER_VARIABLE = '--bwhale-cover'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-bwhale-dock-open'
const DOCK_STORAGE_KEY = 'bwhale'

/**
 * The cover URL, served by the host half at `/skin-cover/bwhale.webp` (see COVER_ROUTE in src/index.ts).
 *
 ...
 */
const COVER_URL = '/skin-cover/bwhale.webp?v=0.1.1'
...
const BRAND_SLOTS = [
  ...
  { name: 'sidebar.brand.name', component: BwhaleName },
...
export interface Config {
  /**
   * Switch to Blue Whale Ocean on install; on by default.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * Mounting does not depend on whether the skin is active; visibility is left to CSS (display only under `body[data-dsh-bwhale]`) —
  ...
  ctx.effect(() => ctx.slots.inject('conversation.composer.dock', () => ctx.slots.register({
    ...
    order: 100,
  }, BwhaleStatusProbe)), 'bwhale: status probe')
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'dark', tokens: BWHALE_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'bwhale: theme + cover + brand')
}
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[bwhale] another skin already claimed the auto-apply slot (%s); this one waits to be picked', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   * (`isThemePreference('bwhale')` is false and third-party ids never reach persistence), and when the Host snapshot
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[bwhale] auto-apply failed; you can switch manually under Settings → Skin Market', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  root.render(createElement(BwhaleStatusDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-bwhale-dock-open')
  }
}
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[bwhale] brand slot %s takeover failed; keeping the official mark', slot.name, error)
        return () => {}
      }
    }))
  }
