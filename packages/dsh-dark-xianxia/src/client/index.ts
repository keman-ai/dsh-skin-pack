...
import { XianMark, XianName } from './Brand.tsx'
import { XianStatusDock } from './StatusDock.tsx'
import { XianEnergyRail } from './EnergyRail.tsx'
import { XianStatusProbe } from './StatusProbe.tsx'
import { XIAN_TOKENS } from './tokens.ts'
import './xian.module.css'

export { XIAN_PALETTE, XIAN_TOKENS } from './tokens.ts'

/** Theme id, and the argument to `setTheme`. Three ids must agree: this constant, skin.json and cordis.patch.yml. */
export const THEME_ID = 'xian'
 ...
 */
const DOCK_OPEN_ATTRIBUTE = 'data-xian-dock-open'
const DOCK_STORAGE_KEY = 'xian'

/**
 * The cover URL, served by the host half at `/skin-cover/xian.webp` (see COVER_ROUTE in src/index.ts).
 *
 ...
 */
const COVER_URL = '/skin-cover/xian.webp?v=0.1.1'
 ...
 */
const BRAND_SLOTS = [
  { name: 'sidebar.brand.mark', component: XianMark },
  { name: 'sidebar.brand.name', component: XianName },
  { name: 'conversation.hero.brand.mark', component: XianMark },
] as const
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   *
   * Mounting does not depend on whether the skin is active; visibility is left to CSS (display only under `body[data-dsh-xian]`) —
   * The rule is "not active means not present", so no half-built UI shows during the window before the skin takes effect.
   */
  ctx.effect(() => mountDock(), 'xian: status dock')
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
    ...
    id: 'xian-energy',
    order: 100,
  }, XianEnergyRail)), 'xian: energy rail')
  ...
  ctx.effect(() => {
    const unregister = ctx.theme.register({ id: THEME_ID, colorScheme: 'dark', tokens: XIAN_TOKENS })
    const unmount = mountStage(ctx, shouldAutoApply(ctx, config.autoApply !== false), userPicked())
    return () => {
      unmount()
      unregister()
    }
  }, 'xian: theme + cover + brand')
}
...
function shouldAutoApply(ctx: Context, configured: boolean): boolean {
  ...
  if (scope[CLAIM_KEY] !== undefined) {
    ctx.logger.info('[xian] another skin already claimed the auto-apply slot (%s); this one waits to be picked', String(scope[CLAIM_KEY]))
    return false
  }
...
function mountStage(ctx: Context, autoApply: boolean, picked: boolean): () => void {
   ...
   * (`isThemePreference('xian')` is false and third-party ids never reach persistence), and when the Host snapshot arrives
  ...
  const sync = (): void => {
    ...
    if (ctx.theme.getTheme().active.id !== THEME_ID && autoApply && !settled && mayApply) {
      try {
        ...
        ctx.logger.warn('[xian] auto-apply failed; you can switch manually under Settings → Skin Market', error)
      }
      return
    }
...
function mountDock(): () => void {
  ...
  host.setAttribute('data-xian-dock', '')
  document.body.append(host)
  const root = createRoot(host)
  root.render(createElement(XianStatusDock))
  return () => {
    // Unmount asynchronously: React forbids a synchronous unmount inside its own render cycle.
    queueMicrotask(() => { root.unmount() })
    host.remove()
    document.body.removeAttribute('data-xian-dock-open')
  }
}
  ...
  for (const slot of BRAND_SLOTS) {
    disposers.push(ctx.slots.inject(slot.name, () => {
      ...
      try {
        ...
        ctx.logger.warn('[xian] brand slot %s takeover failed; keeping the official mark', slot.name, error)
        return () => {}
      }
    }))
  }
