/**
 * The right-hand dock: a reduced night-forest cover plus this session's real state.
 *
...
export function NforestStatusDock() {
      ...
      {open && (
        <div className={css.body}>
          ...
          <div className={css.scroll}>
            ...
            */}
            <div className={css.cover} style={{ backgroundImage: 'var(--nforest-cover)' }}>
              <span className={css.coverName}>Night Forest Companion</span>
            </div>
            ...
            */}
            ...
            {/*
              Night-walk log — the real-data version of that card in the prototype's right column.
            ...
            */}
            {toolCalls.length > 0 && (
              <section className={css.card}>
                <div className={css.cardTitle}>Night-walk log</div>
                  ...
                  {toolCalls.map((call, index) => (
                    <li key={`${call.name}-${index}`} className={css.logRow} data-state={callState(call)}>
                      ...
                      <span className={css.logMeta}>
                        {call.running === true
                          ? `walking · ${formatDuration(Math.max(0, now - (call.startedAt ?? now)))}`
                          : call.failed === true
                            ? call.ms === undefined ? 'lost' : `lost · ${formatDuration(call.ms)}`
                            : call.ms === undefined ? 'arrived' : `arrived · ${formatDuration(call.ms)}`}
                      </span>
                    </li>
                  ))}
                </ul>
                {moreTools > 0 && <p className={css.hint}>{`${moreTools} earlier walk(s) not shown`}</p>}
                {/*
