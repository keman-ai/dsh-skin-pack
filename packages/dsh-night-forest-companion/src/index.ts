/**
 * The Night Forest Companion skin · host half.
 *
 ...
 *
 * @module dsh-night-forest-companion
 */

import type { Context } from '@deepseek-ai/cordis'
import type { IncomingMessage, ServerResponse } from 'node:http'
import { NFOREST_COVER } from './cover.generated.ts'

/** Plugin name (the `name` of the loader entry). */
export const name = 'nforest'
 ...
 */
export const COVER_ROUTE = '/skin-cover/nforest.webp'

/** Cover bytes. The data URI is decoded once on this side and kept in memory (a few hundred KB, immaterial on the Node side). */
const COVER_BYTES = Buffer.from(NFOREST_COVER.slice(NFOREST_COVER.indexOf(',') + 1), 'base64')
...
export interface Config {
  /**
   * Switch to Night Forest Companion on install; on by default. Turned off, it only registers without applying, leaving the user to pick it under Settings → Appearance.
   *
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.logger.info('[nforest] Night Forest Companion mounted (%s)', mode)
}
